...
class RxMapperImpl(private val client: Client, private val currencyCreator: CurrencyCreator) : RxMapper {

    override fun getRates(baseSymbol: String, baseAmount: BigDecimal): Observable<List<Currency>> {
        ...
        return client.getNames()
            .flatMap { names ->
                client.getRates(baseSymbol)
                    .map { response ->
                        currencyCreator.createRates(response, names, baseAmount)
                    }
